...
function Pagination({ currentPage, onClick, maxPage }) {
  ...
  const handleArrowClick = (index) => {
    console.log('click arrow', index);
    console.log(currentPage, referenceNum);
    if (index === 0) {
      // 1. 좌측 화살표 클릭
      // 첫 페이지에서는 작동 금지
      if (referenceNum >= 1) {
  ...
  const isDisableLeft = referenceNum < 1;
  const isDisableRight = referenceNum >= Math.ceil(maxPage / PAGE_LENGTH) - 1;
